...
def model_mimlcnn(datasets, Wordv, PF1v, PF2v, img_h, RForAT, WForAT, WForATData, BForATData, linearW, WForEP, BForEP,
                  WForFianl):
    """
    模型建模.
    ...
    def atData(input, left, right, Slen):
        ...
        def AtLayerData(LRConnect):
            ...
            def punish(item, i):
                item = T.switch(T.lt(i, 4), item - 9999, item)
                item = T.switch(T.gt(i, Slen + 1), item - 9999, item)
                return item
    ...
    [myobser1, myob], _ = theano.scan(atData, sequences=[cp_layer_input, e1is, e2is, Slens])
    ...
    # changeInput1 = 26×50×(690+120)
    def change1(index, changeInput1):

        def change2(changeInput2):
            return changeInput2[index]

        # 26×(690+120)
        aa, _ = theano.scan(change2, sequences=[changeInput1])
        return aa

    # 50×26×(690+120)
    okchange, _ = theano.scan(change1, sequences=[my50], non_sequences=[ok])
    ...
    # 50×26×(690+120)
    oksecond, _ = theano.scan(finalLayer4, sequences=[okchange])

    # 之后再转回26×50×(690+120)

    # changeInput3 = 50×26×(690+120)
    def change3(index3, changeInput3):

        def change4(changeInput4):
            return changeInput4[index3]

        # 50×(690+120)
        aa, _ = theano.scan(change4, sequences=[changeInput3])
        return aa

    # 26×50×(690+120)
    okorigin, _ = theano.scan(change1, sequences=[idx], non_sequences=[oksecond])

    # (26, 50, (690 + 120))
    normalre, dropoutre = my_mlp_layer.feed(idx, okorigin,
    ...
    op_params += [WForAT, RForAT, WForATData, BForATData, linearW, WForEP, BForEP, WForFianl]
    params += [WForAT, RForAT, WForATData, BForATData, linearW, WForEP, BForEP, WForFianl]
    ...
    train_update_model = theano.function([bch_idx], output_list, updates=grad_updates,
                                         ...
                                         givens={
                                             ...
                                             my50: np.arange(50, dtype='int32'),
                                             manyi: np.arange(img_h - 2, dtype='int32'),
                                         }, )
...
def run_mimlcnnre(file_name):
    ...
    WForFianl_init = np.asarray(rng.uniform(low=-W_bound, high=W_bound, size=[my_in, my_out]),
                                dtype=theano.config.floatX)
    ...
    WForFianl = theano.shared(value=WForFianl_init, name="linearW");
    # *****************
    ...
    train_update_model, trainset_error_model, testset_error_model, set_zero, init_LR_W, init_LR_b = \
        model_mimlcnn(datasets_ready, Wordv, PF1v, PF2v, img_h, RForAT, WForAT, WForATData, BForATData, linearW, WForEP,
                      BForEP, WForFianl)
    # 6. 训练与验证效果
