...
from theano_layers import MyLeNetConvPoolLayer, MyMLPDropout, MLPDropout, TwoConvPoolLayers
import sys
import datetime
import ConfigParser
import argparse
...
def model_mimlcnn(datasets, Wordv, PF1v, PF2v, img_h, WForATData, linearW):
    ...
    # 例子数×(690+120)
    sec_maxPooling_out, _ = theano.scan(fn=ep_max_pooling, sequences=ep2m, non_sequences=new_cp_out)

    logging.info('  - Defining MLP layer')
    if not _USE_STACK_CP and _USE_PIECEWISE_POOLING_41CP:
        _MLP_SHAPE[0] *= 3
        _MLP_SHAPE[0] = _MLP_SHAPE[0] + 2 * _IMG_W
    ...
    mlp_layer.feed(sec_maxPooling_out, input_shape=(_BATCH_SIZE, _MLP_SHAPE[0]))
...
def run_mimlcnnre(file_name):
    ...
    train_update_model, trainset_error_model, testset_error_model, set_zero, init_LR_W, init_LR_b = \
        model_mimlcnn(datasets_ready, Wordv, PF1v, PF2v, img_h, WForATData, linearW)
