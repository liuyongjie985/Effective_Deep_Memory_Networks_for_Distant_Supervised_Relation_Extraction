...
def model_mimlcnn(datasets, Wordv, PF1v, PF2v, img_h, RForAT, WForAT, WForATData, BForATData, linearW, WForEP, BForEP):
    ...
    if not _USE_STACK_CP and _USE_PIECEWISE_POOLING_41CP:
        # _MLP_SHAPE = [120, 2]
        _MLP_SHAPE[0] = _IMG_W * 2
        logging.info("    - MLP shape changes to {0}, because of piecewise max-pooling".format(_MLP_SHAPE))

    # _MLP_SHAPE = [690,26] _MLP_ACTIVATIONS = [Iden] dropout_rates = [0.5]


    my_mlp_layer = MyMLPDropout(rng, layer_sizes=[[_MLP_SHAPE[0], 2]], activations=_MLP_ACTIVATIONS,
    ...
    normalre, dropoutre = my_mlp_layer.feed(idx, ok,
                                            input_shape=(_N_RELATIONS, _BATCH_SIZE, (2 * _IMG_W)))
    ...
    output_list = [cost, dropout_cost, myob]
    ...
    while epoch <= _N_EPOCHS:
        ...
        for batch_index in index_seq_to_train:
            if not conf.getboolean('mode', 'output_details'):
                cost, dropout_cost, myob = train_update_model(batch_index)
                # print myob.shape
                # print myob
            else:
